...
def detectLine(frame):
    ...
    roi_C = frame[
        340 : 340 + Gap, 100 : Width - 100
    ]  # 센터라인용 roi인데 실제로 돌려보고 조정해도 괜츈(roi더 넓게 하되 이전 좌표와의 편차로 벗어나는 값들 거르기)
    # roi_C = frame[340:340+Gap, x_C:x_C+w]
...
def posCalibration(lane_num):
    ...
    if start_obstacle == 2:
        if lane_num == 0 and (cpos - lpos < 100 or cpos - lpos > 250):
            ...
            rpos = lpos + 280
            print("---------------------------------google------------------------------------")
    if lane_num == 1 and (cpos - lpos < 100 or cpos - lpos > 250):
        cpos = lpos + 140
        rpos = lpos + 280
        print("---------------------------------naver------------------------------------")
    elif lane_num == 2 and (rpos - cpos < 100 or rpos - cpos > 250):
        cpos = rpos - 140
        lpos = rpos - 280
        print("---------------------------------kakao------------------------------------")
